...
for k = 1:K  %for each group
    ...
    for e=1:emotions
        ...
        trees(k,e).tree = ConstructDecisionTree(x(train_set,:), ny(train_set,e),0,75); %learns with the 9 other groups
        %DrawDecisionTree(trees(e).tree,num2str(e));                              
        
        output(e,test_set) = testTree(trees(k,e).tree, x(test_set,:));  %tests with the test group
