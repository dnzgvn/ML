...
for f = 1:x_feature   
    ...
    for sF = 1:(u_sample-1)
        ...
        SamplesLargerThan = find(features(:,f)>minValue);  %indices of samples larger than threshold
        SamplesLessThan = find(features(:,f)<=minValue);  %indices of samples less than threshold
        ...
        SplitInfoGain(sF,2) = minValue;  %threshold
